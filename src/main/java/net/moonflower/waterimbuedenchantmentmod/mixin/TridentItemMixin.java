package net.moonflower.waterimbuedenchantmentmod.mixin;
...
public class TridentItemMixin extends Item implements Vanishable {
    ...
    /**
     * @author themoonflowerteam
     * @reason water imbued enchantment is supposed to let you use riptide without touching water
     */
    @Overwrite
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        if (user instanceof PlayerEntity playerEntity) {
            ...
            if (i >= 10) {
                ...
                if (j <= 0 || playerEntity.isTouchingWaterOrRain() || isWaterImbued(stack)) {
                    if (!world.isClient) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        } else if (EnchantmentHelper.getRiptide(itemStack) > 0 && !user.isTouchingWaterOrRain() && !isWaterImbued(itemStack)) {
            return TypedActionResult.fail(itemStack);
        } else {
    ...
    @Unique
    public boolean isWaterImbued(ItemStack stack) { return EnchantmentHelper.getLevel(ModEnchantments.WATER_IMBUED, stack) > 0; }
